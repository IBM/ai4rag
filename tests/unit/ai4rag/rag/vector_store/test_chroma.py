...
class TestChromaVectorStoreAddDocuments:
    ...
    def test_add_documents_with_default_max_batch_size(self, mocker, mock_embedding_model):
        """Test add_documents uses default max_batch_size of 2048 from kwargs."""
        mock_client = MagicMock()
        mock_client.get.return_value = {"ids": []}
        ...
        content = ["Doc 1"]
        result = vector_store.add_documents(content)
        # With only 1 doc and default max_batch_size=2048, should call add_documents once
        assert isinstance(result, list)
...
class TestChromaVectorStoreGetWindowDocuments:
    ...
    def test_get_window_documents_preserves_full_metadata(self, mocker, mock_embedding_model):
        """Test _get_window_documents preserves all metadata fields (e.g., headers)."""
        mock_client = MagicMock()
        mock_client.get.return_value = {
            "documents": ["Doc 1", "Doc 2"],
            "metadatas": [
                {"sequence_number": 1, "document_id": "doc1", "Header 1": "Intro", "Header 2": "Setup"},
                {"sequence_number": 2, "document_id": "doc1", "Header 1": "Intro", "Header 2": "Usage"},
            ],
        }
        mocker.patch("ai4rag.rag.vector_store.chroma.Chroma", return_value=mock_client)
        vector_store = ChromaVectorStore(embedding_model=mock_embedding_model)
        result = vector_store._get_window_documents("doc1", [1, 2])
        assert result[0].metadata["Header 1"] == "Intro"
        assert result[0].metadata["Header 2"] == "Setup"
        assert result[1].metadata["Header 2"] == "Usage"
