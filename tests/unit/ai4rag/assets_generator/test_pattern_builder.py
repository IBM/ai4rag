...
class TestBuildPatternJson:
    ...
    def test_export_system_input_merges_non_redundant_user_rules(self):
        """User supplements merge when system lacks grounding; redundant grounding is skipped."""
        pattern = _make_pattern()
        ...
        assert "retrieved via file search" in system_text
        ...
        assert "Answer (max 150 words, with citations):" in system_text
    ...
    def test_export_system_input_skips_duplicate_citation_and_keeps_answer_scaffold(self):
        """Citation already in system must not duplicate; answer scaffold still merges."""
        pattern = _make_pattern()
        pattern["settings"]["generation"]["system_message_text"] = (
    ...
    def test_export_system_input_uses_default_model_props(self):
        """Default PR #75 templates export without OGX-duplicative citation instructions."""
        model_id = "ibm/granite-3-8b-instruct"
        pattern = _make_pattern()
        ...
        assert "specialized Retrieval Augmented Generation" not in system_text
        assert "[1], [2]" not in system_text
        assert "max 150 words" in system_text
        assert "English only" in system_text
        assert "{reference_documents}" not in system_text
    ...
    def test_build_responses_system_input_legacy_user_prefix(self):
        """Legacy patterns without system grounding get Responses-native merged supplements."""
        generation = {
        ...
        assert system_input.startswith("Short system prefix.")
        assert "retrieved via file search" in system_input
        assert "must cite sources" not in system_input.lower()
        assert "file citations" not in system_input.lower()
        assert "documents below" not in system_input
        assert "[1], [2]" not in system_input
    ...
    def test_build_pattern_json_uses_export_parity_system_input(self):
        ...
        assert "max 150 words" in actual
