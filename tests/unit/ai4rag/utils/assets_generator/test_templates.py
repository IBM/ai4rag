...
class TestGeneratedNotebookUsesDirectClients:
    ...
    def test_generated_notebook_uses_single_maas_client(self, template: str, tmp_path: Path):
        ...
        # A single MaaS client is built from the env-provided base URL, once, via the
        # shared factory (which also handles the self-signed-cert TLS fallback).
        assert "client = create_maas_client(base_url=MAAS_BASE_URL, api_key=MAAS_API_KEY)" in text
        assert text.count("create_maas_client(") == 1
