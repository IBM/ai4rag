...
class TestIsSslError:
    ...
    def test_detects_certificate_verify_failed(self):
        ...
        from ai4rag.components.utils.maas_client import is_ssl_error
...
class TestCreateMaasClient:
    ...
    def test_falls_back_to_unverified_tls_on_ssl_error(self, mocker):
        ...
        mock_openai_cls = mocker.patch("ai4rag.components.utils.maas_client.OpenAI")
        mock_httpx_client = mocker.patch("ai4rag.components.utils.maas_client.httpx.Client")
    ...
    def test_falls_back_on_chained_ssl_error(self, mocker):
        ...
        mock_openai_cls = mocker.patch("ai4rag.components.utils.maas_client.OpenAI")
        mocker.patch("ai4rag.components.utils.maas_client.httpx.Client")
    ...
    def test_logs_warning_on_ssl_fallback(self, mocker):
        ...
        mock_openai_cls = mocker.patch("ai4rag.components.utils.maas_client.OpenAI")
        mocker.patch("ai4rag.components.utils.maas_client.httpx.Client")
        mock_logger = mocker.patch("ai4rag.components.utils.maas_client._logger")
...
class TestMaasModelBaseUrl:
    """Test suite for :func:`maas_model_base_url`."""

    def test_derives_per_model_url_from_general_url(self):
        """The per-model URL is ``{scheme}://{netloc}/{owned_by}/v1``."""
        from ai4rag.components.utils.maas_client import maas_model_base_url

        assert (
            maas_model_base_url("https://host.example.com/maas-api/v1", "ai-eng-cracow/bge-m3")
            == "https://host.example.com/ai-eng-cracow/bge-m3/v1"
        )

    def test_discards_general_url_path(self):
        """Any path on the general URL is dropped; only scheme and host are reused."""
        from ai4rag.components.utils.maas_client import maas_model_base_url

        assert maas_model_base_url("http://host/anything", "ns/model") == "http://host/ns/model/v1"

    def test_strips_surrounding_slashes_on_owned_by(self):
        """Leading/trailing slashes on ``owned_by`` are normalized away."""
        from ai4rag.components.utils.maas_client import maas_model_base_url

        assert maas_model_base_url("https://host/maas-api/v1", "/ns/model/") == "https://host/ns/model/v1"

    def test_accepts_anyurl_object(self):
        """A non-string base URL (e.g. ``AnyUrl``) is coerced to text first."""
        from pydantic import AnyUrl

        from ai4rag.components.utils.maas_client import maas_model_base_url

        assert (
            maas_model_base_url(AnyUrl("https://host.example.com/maas-api/v1"), "ns/model")
            == "https://host.example.com/ns/model/v1"
        )


class TestCreateMaasModelClient:
    """Test suite for :func:`create_maas_model_client`."""

    def test_returns_plain_client_without_probe(self, mocker):
        """Per-model clients are created without a ``models.list()`` connectivity probe."""
        mock_openai_cls = mocker.patch("ai4rag.components.utils.maas_client.OpenAI")
        mock_client = mocker.MagicMock()
        mock_openai_cls.return_value = mock_client
        ...
        result = create_maas_model_client(base_url="https://host/ns/model/v1", api_key="k")

        assert result is mock_client
        mock_openai_cls.assert_called_once_with(base_url="https://host/ns/model/v1", api_key="k")
